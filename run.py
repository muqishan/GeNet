...
if __name__ == "__main__":
    DEFAULT_RUN_ARGS = {
    "task_name": "UniRel",
    "model_dir": "./pubmedbert/",#pubmedbert
    "dataset_dir": "datasets",
    "dataset_name": "my_data",
    "test_data_type": "unirel_span",
    "threshold": 0.5,
    "max_seq_length": 180,
    "do_test_all_checkpoints": True,
        }
    ...
    if len(sys.argv) == 1:  # 如果没有提供命令行参数
        run_args = RunArguments(**DEFAULT_RUN_ARGS)
        training_args = TrainingArguments(**DEFAULT_TRAINING_ARGS)
    elif len(sys.argv[1]) == 2 and sys.argv[1].endswith(".json"):
